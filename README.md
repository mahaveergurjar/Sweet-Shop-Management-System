...
4. **Set up the database:**
   ...
   Ensure you have a PostgreSQL database running locally or set up via the cloud (like Supabase or Neon), and that its connection string is in your backend `.env` file under `DATABASE_URL`.

   ```bash
   cd backend
   # Push schema to the database
   npm run migrate:dev
   # Seed the database with authentic demo data and images
   npm run db:seed
   ```
...
- View all available sweets (with images directly from Unsplash)
- Search sweets by name, category, or price range
- Understand pricing by weight/format with dynamic `unit` handling (kg, box, piece)
- Purchase sweets (decreases quantity safely)
- Admin-only: Add, update, and delete sweets
- Admin-only: Restock sweets

### User Interface

- Modern, responsive design visually enriched
- Real-time inventory updates with flash notifications via `react-hot-toast`
...
This project follows a component-driven design built aggressively with **Continuous Verification**:

- Fast local setup via Prisma commands
